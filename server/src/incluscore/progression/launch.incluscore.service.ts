...
import {
  LaunchIncluscoreDb,
  LaunchIncluscoreDocument,
} from "../entities/launch.incluscore.entity";
import { SaveLaunchIncluscoreDto } from "../dto/creation/save.launch.incluscore.dto";
import {
  UserThemeIncluscoreDb,
  UserThemeIncluscoreDocument,
} from "../entities/userTheme.entity";
import * as mongoose from "mongoose";
import {
  UserAnswerIncluscoreDb,
  UserAnswerIncluscoreDocument,
} from "../entities/userAnswer.entity";
...
export class LaunchIncluscoreService {
  ...
  async findAllForAdminCompanyEditionPage(
    idCompany: string
  ): Promise<LaunchIncluscoreDb[]> {
    return this.launchScrDb
      .find(
        { idCompany: idCompany },
        { idTeam: 1, idIncluscore: 1, userThemes: 1 }
      )
      .populate({
        path: "idIncluscore",
        ...
        populate: {
          path: "themes",
          select: "name",
          populate: { path: "questions", select: "_id title" },
        },
      })
      .populate({
        path: "userThemes",
  ...
  async addUserThemeIfNotExist(
    userTheme: UserThemeIncluscoreDb
  ): Promise<void> {
  ...
  async fixDuplicateThemes(): Promise<string[]> {
    ...
    const usersThemesPossiblyWrong: UserThemeIncluscoreDb[] =
      await this.userThemeIncluscoreDb
    ...
    for (const userTheme of usersThemesPossiblyWrong) {
      ...
      if (!firstDuplicate) {
        ...
        fixedUserThemes = fixedUserThemes.map((newU: UserThemeIncluscoreDb) =>
          (newU as UserThemeIncluscoreDocument)._id.equals(firstDuplicate?._id)
            ? userTheme
            : newU
        );
      }
    ...
    const userThemesWrongsIds = usersThemesPossiblyWrong
      .filter(
        (u) =>
          !fixedUserThemes.find((f) =>
            (f as UserThemeIncluscoreDocument)._id.equals(u._id)
          )
      )
      .map((ut) => ut._id);
  ...
  async fixDuplicateAnswers(launch: LaunchIncluscoreDb) {
    ...
    const userThemesWithDuplicateQuestion: UserThemeIncluscoreDb[] =
      await this.userThemeIncluscoreDb
        .find({ _id: { $in: result[0].query.map((q) => q.userThemeId) } })
        .populate({
          path: "answers",
    ...
    for (const userThemeWithDuplicateQuestion of userThemesWithDuplicateQuestion) {
      ...
      for (const a of userThemeWithDuplicateQuestion.answers) {
        if (
          fixedAnswers.find((newA) =>
            newA.questionId._id.equals(a.questionId._id)
          )
        ) {
        ...
        if (a?.userAnswer?.isAGoodAnswer) {
          userThemeWithDuplicateQuestion.score += launch.idIncluscore
            .isInclucard
      ...
      await (
        userThemeWithDuplicateQuestion as UserThemeIncluscoreDocument
      ).update({
        $set: {
          answers: fixedAnswers,
          score: userThemeWithDuplicateQuestion.score,
        },
      });
    }
  }
        ...
        { _id: launch._id },
        {
          $set: {
            userThemes: launch.userThemes.filter(
              (ut) =>
                !userThemesWrongsIds.find((id) =>
                  (ut as UserThemeIncluscoreDocument)._id.equals(id)
                )
            ),
          },
        }
