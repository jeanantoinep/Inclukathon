...
import {
  LScrStatService,
  StatsMainObject,
} from "./launch.incluscore.stats.service";
...
export class LaunchScrController {
  ...
  @Post()
  async save(
    @Body() launch: SaveLaunchIncluscoreDto
  ): Promise<LaunchIncluscoreDto[]> {
    launch._id = launch.id;
    await this.launchScrService.save(launch);
    const launches = await this.launchScrService.findAllByCompanyId(
      launch.idCompany
    );
  ...
  async findLaunchForIncluscoreApp(
    ...
    const launch: LaunchIncluscoreDb = await this.launchScrService.findOneLight(
      id,
      currentUserId
    );
    ...
    if (company.teamArborescence) {
      company.teamArborescence = company.teamArborescence.filter((ta) => {
        return company.teams.find(
          (team) =>
            (team as TeamDb).level1?._id?.toString("hex") ===
              ta.id?.toString("hex") ||
            (team as TeamDb).level2?._id?.toString("hex") ===
              ta.id?.toString("hex") ||
            (team as TeamDb).level3?._id?.toString("hex") ===
              ta.id?.toString("hex")
        );
      });
    }
  ...
  async findAllForAdminCompanyEditionPage(
    ...
    const launches: LaunchIncluscoreDb[] =
      await this.launchScrService.findAllForAdminCompanyEditionPage(idCompany);
    ...
    for (const launchDto of launchesDto) {
      if (!launchDto.userThemes?.length) {
        launchDto.userThemes = await this.userThemeService.findByLaunchId(
          launchDto.id
        );
      }
    }
  ...
  async findStatsForLaunchStatPage(
    @Param("idLaunch") idLaunch: string
  ): Promise<BasicFacetStatsDto> {
    const launch: LaunchIncluscoreDb = await this.launchScrService.findForStats(
      idLaunch,
      { idIncluscore: 1 }
    );
    const stat: StatsMainObject =
  ...
  async deleteOne(
    ...
    const companyLaunches = await this.launchScrService.deleteOne(
      id,
      idCompany
    );
