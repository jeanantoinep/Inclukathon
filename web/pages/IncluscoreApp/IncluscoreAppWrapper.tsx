...
class IncluscoreAppWrapper extends Component<
  IProps,
  IIncluscoreAppWrapperState
> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      npsNotation: window.connectedUser
        ? window.connectedUser.npsNotation
        : null,
      launchStats: null,
    };
  }
  ...
  async retrieveIncluscoreByIds(idLaunch: string) {
    ...
    const launch: LaunchIncluscoreDto = await HttpRequester.getHttp(
      `${LAUNCH_SCR_CTRL}/${idLaunch}`
    );
    ...
    if (
      !launch.idIncluscore.canBePublic ||
      this.props.location.pathname !== incluscoreHomePath
    ) {
      this.redirectToLoginPageIfNotConnected(launch.idIncluscore.canBePublic);
    }

    const launchStats = await HttpRequester.getHttp(
    ...
    hideLoader(this.constructor.name);
    if (
      launch.idIncluscore.isInclucard &&
      !this.props.history.location.pathname.includes(inclucardAppPath)
    ) {
  ...
  setContext = () => {
    ...
    if (this.state.incluscore && this.state.incluscore.isInclucard) {
      ...
      root.style.setProperty(
        "--inclucard-main-color",
        this.state.incluscore.inclucardColor
      );
      ...
      if (this.state.incluscore.incluscoreColor) {
        root.style.setProperty(
          "--incluscore-main-color",
          this.state.incluscore.incluscoreColor
        );
        root.style.setProperty(
          "--incluscore-second-color",
          this.state.incluscore.incluscoreColor
        );
      }
      if (this.state.incluscore.secondIncluscoreColor) {
        root.style.setProperty(
          "--incluscore-second-color",
          this.state.incluscore.secondIncluscoreColor
        );
      }
    }
  };
  ...
  sendNpsComment = async () => {
    ...
    window.localStorage.setItem(
      "dont-show-nps-" + window.connectedUser.id,
      "true"
    );
    ...
    return (
      <div
        className={`nps-container ${
          themesPage ? "animate__animated animate__fadeInBottomRight" : ""
        }`}
      >
              ...
              {this.state.npsNotation < 7 ? (
                ...
                <>
                  {this.state.npsNotation < 9 && (
                    <Translation ns={["translation", "incluscore"]}>
                      {(t) => (
                        <>{t("nps.whyXpWasMedium", { ns: "incluscore" })}</>
                      )}
                    </Translation>
                  )}
                  {this.state.npsNotation > 8 && (
                    <Translation ns={["translation", "incluscore"]}>
                      {(t) => (
                        <>{t("nps.whyXpWasGood", { ns: "incluscore" })}</>
                      )}
                    </Translation>
                  )}
                </>
      ...
      <div className={"incluscore-app-wrapper"}>
        {(this.state.incluscore || this.props.isErrorPage) &&
          React.cloneElement(child, {
            ...
            launchStats: this.state.launchStats,
  ...
  componentDidUpdate() {
    ...
    const hasBg = document
      .querySelector("body")
      .classList.contains("inclucard-background");
